...
// ───────────────────────────────────────────────────────────────
// loyaltyRules 変換は v6 PR-1e で廃止。LoyaltyRule 型を物理削除したため、
// 抽出された loyaltyRules は propose では無視される (extracted キャッシュとの
// 互換で受理はするが提案には変換しない)。point-partner を programs+memberships
// 直接出力へ移す 1f のプロンプト総改訂後、本経路と ExtractedSource.loyaltyRules を
// まとめて撤去する。rate→program-id の決定論採番 (rateToProgramSlug /
// deriveLoyaltyProgramId) はその移行で再利用するため上に温存している。
// ───────────────────────────────────────────────────────────────
