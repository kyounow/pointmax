...
### キャンペーン（期間限定ルール）
- 開始日・終了日付き、または**毎月の日にち指定**（`recurringDays`、例:「5と0のつく日」= [5,10,15,20,25,30]）の
  BenefitProgram を表現できる（曜日指定は未対応）。
- 通常プログラムと共存し、期間中のみ有効化（`primary` は最高 rate を採用、`addOn` は上乗せ）。
- 計算結果にキャンペーンバッジを表示。専用タブで「有効中／期限切れ／未来開始」を確認でき、
  「期限あり」タブは**終了が近い順**に並び、終了日に「あと N 日」カウントダウンを表示。
  計算画面の今日バナーにも「⌛ まもなく終了 N 件」(7 日以内) が出る。
- 「今日」の判定は日付が変わると自動で更新される（開きっぱなしの PWA でも
  キャンペーンの開始・終了・「5のつく日」が翌日に正しく切り替わる）。
- キャンペーンの登録・更新は現在マスターデータ (seed + 週次自動同期) でのみ管理
  （アプリ内の手動登録 UI は v3 で廃止、将来再提供を検討）。
