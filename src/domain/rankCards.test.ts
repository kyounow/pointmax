...
// fix-fallback-addon: 互換 paymentApp が 0 件になる分岐 (compatibleApps.length===0) でも
// addOn を通常分岐と同一処理で加算することの回帰テスト。
// 旧実装はこの分岐で appBonus 系を 0 / [] 固定にしていたため、決済アプリを 1 つも有効化して
// いないユーザー (v7 全OFF 出荷後) で「5と0のつく日」+1% 等の上乗せが丸ごと落ちていた。
// 注意: paymentApps=[] だと別分岐 (paymentApp なし) を通るため、このバグの回帰にならない。
// 「paymentApps は存在するが全 disabled」= 全OFF 出荷相当で確実にフォールバック分岐を踏む。
describe("fix-fallback-addon: 互換 paymentApp 0 件分岐でも addOn を加算", () => {
  const rakutenCard: Card = {
    id: "rakuten-card",
    name: "楽天カード",
    defaultRate: 0.01,
  ...
  const rakutenIchiba: Store = { id: "rakuten-ichiba", name: "楽天市場" };
  // 全 disabled paymentApp → compatibleApps 0 件 → フォールバック分岐。
  const allOffPaymentApps: PaymentApp[] = [
    { id: "pa-rpay", name: "楽天ペイ", enabled: false },
  ];
  const zeroFiveDay = new Date(2026, 6, 25); // 2026-07-25 = 5 の付く日 (recurringDays に該当)
  const nonZeroFiveDay = new Date(2026, 6, 23); // 2026-07-23 = 対象外日

  // withHeldDefaults を経由せず rankCards を直接呼ぶ: enabled:false を尊重させ、
  // フォールバック分岐を確実に踏むため (ラッパは enabled 未指定を true に上書きする)。
  function rankFallback(amount: number, now: Date) {
    return rankCards({
      payment: { storeId: "rakuten-ichiba", amount },
      targetCurrencyId: "rakuten-pt",
      cards: [rakutenCard],
  ...
  it("全OFF (paymentApps 全 disabled) + 5と0の日: base 3% + addOn 1% = 実質4%", () => {
    const [row] = rankFallback(50000, zeroFiveDay);
    // paymentApp 経由ではないので header の決済アプリは null。
    expect(row.paymentApp).toBeNull();
    // primary は base 3% (SPU 基本)。
    expect(row.resolved.source).toBe("program");
    expect(
      row.resolved.source === "program" ? row.resolved.programId : null,
    ).toBe("prog-rakuten-ichiba-base");
    ...
    // ★本 fix の回帰基準: フォールバック分岐でも「5と0」+1% addOn が乗る。
    const zeroFive = row.appBonusBreakdown.find(
      (b) => b.programId === "prog-rakuten-ichiba-zero-five-day",
    );
    expect(zeroFive, "フォールバック分岐で 5と0 addOn が落ちている").toBeDefined();
    expect(zeroFive?.rate).toBe(0.01);
    expect(zeroFive?.finalAmount).toBeCloseTo(50000 * 0.01, 6); // +1% = 500
    expect(row.appBonusFinalAmount).toBeCloseTo(500, 6);
    expect(row.appBonusRate).toBeCloseTo(0.01, 10);
    expect(row.appBonusReachable).toBe(true);
    // 合計は実質 4% (base 1500 + addOn 500 = 2000)。
    expect(row.totalFinalAmount).toBeCloseTo(2000, 6);
  });

  it("全OFF + 非対象日: addOn 不発で base 3% のみ", () => {
    const [row] = rankFallback(50000, nonZeroFiveDay);
    expect(row.paymentApp).toBeNull();
    expect(row.earnedAmount).toBeCloseTo(50000 * 0.03, 6); // base 3% は常時
    const zeroFive = row.appBonusBreakdown.find(
      (b) => b.programId === "prog-rakuten-ichiba-zero-five-day",
    );
    expect(zeroFive, "非対象日なのに 5と0 addOn が出ている").toBeUndefined();
    expect(row.appBonusFinalAmount).toBe(0);
    expect(row.appBonusReachable).toBe(false);
    expect(row.totalFinalAmount).toBeCloseTo(1500, 6); // 3% のみ
  });

  it("全OFF + 上限超 (20万円): addOn のみ cap クランプ、base 3% は無傷", () => {
    const [row] = rankFallback(200000, zeroFiveDay);
    expect(row.earnedAmount).toBeCloseTo(200000 * 0.03, 6); // base 6000 (非クランプ)
    const zeroFive = row.appBonusBreakdown.find(
      (b) => b.programId === "prog-rakuten-ichiba-zero-five-day",
    );
    expect(zeroFive?.finalAmount).toBeCloseTo(100000 * 0.01, 6); // 1000 = 獲得上限
    expect(row.totalFinalAmount).toBeCloseTo(6000 + 1000, 6);
  });

  it("paymentAppId 付き addOn はフォールバック分岐 (DIRECT 評価) では加算されない", () => {
    // DIRECT 評価では paymentAppId 付き program は evaluatePrograms 内で除外される
    // (paymentApp.id="__direct__" と不一致) ため、フォールバック分岐に返る addOns は
    // paymentApp 非依存 (cardIds 系) のみ。rankCards 側の追加フィルタは不要 = それを固定する。
    const card: Card = {
      id: "c-fb",
      name: "Cカード",
      defaultRate: 0.01,
    ...
    const store: Store = { id: "s-fb", name: "S店" };
    ...
    const addOnFree: BenefitProgram = {
      id: "prog-fb-addon-free",
      name: "addOn (paymentApp 非依存)",
      scope: "all-stores",
      cardIds: ["c-fb"],
      rate: 0.01,
      currencyId: "pt",
    ...
    const addOnPaymentApp: BenefitProgram = {
      id: "prog-fb-addon-payapp",
      name: "addOn (paymentApp 専用)",
      scope: "all-stores",
      cardIds: ["c-fb"],
      paymentAppId: "pa-x", // DIRECT 評価では発火しない
      rate: 0.05,
      currencyId: "pt",
      bonusType: "addOn",
    };
    const [row] = rankCards({
      payment: { storeId: "s-fb", amount: 10000 },
      targetCurrencyId: "pt",
      cards: [card],
      stores: [store],
      edges: [],
      paymentApps: [{ id: "pa-x", name: "X Pay", enabled: false }], // 全 disabled → フォールバック
      programs: [basePrimary, addOnFree, addOnPaymentApp],
      memberships: [],
    }).rankings;

    // paymentApp 非依存 addOn (+1%) は乗る。
    expect(
      row.appBonusBreakdown.find((b) => b.programId === "prog-fb-addon-free"),
      "paymentApp 非依存 addOn が落ちている",
    ).toBeDefined();
    // paymentAppId 付き addOn (+5%) はフォールバック分岐で乗らない。
    expect(
      row.appBonusBreakdown.find((b) => b.programId === "prog-fb-addon-payapp"),
      "paymentAppId 付き addOn がフォールバック分岐で加算されている",
    ).toBeUndefined();
    expect(row.appBonusFinalAmount).toBeCloseTo(100, 6); // 10000 * 0.01 のみ
    expect(row.totalFinalAmount).toBeCloseTo(300, 6); // base 200 + addOn 100 (5% は含まない)
  });

  it("同一 fixture: 通常分岐 (paymentApp 有効) と フォールバック分岐 で base+addOn が一致", () => {
    const base = {
      payment: { storeId: "rakuten-ichiba", amount: 50000 },
      targetCurrencyId: "rakuten-pt",
      cards: [rakutenCard],
      stores: [rakutenIchiba],
      edges: [] as ConversionEdge[],
    ...
    // 汎用 (非 chargeBased・どの program の paymentAppId にも一致しない) paymentApp を有効化 → 通常分岐。
    // base/zero-five は paymentAppId を持たないので通常分岐でも同じく発火する。
    const [normal] = rankCards({
      ...base,
      paymentApps: [{ id: "pa-generic", name: "汎用Pay", enabled: true }],
    }).rankings;
    // 同じ paymentApp を disabled にするだけ → フォールバック分岐。
    const [fallback] = rankCards({
      ...base,
      paymentApps: [{ id: "pa-generic", name: "汎用Pay", enabled: false }],
    }).rankings;

    expect(normal.paymentApp?.id).toBe("pa-generic"); // 通常分岐は決済アプリ採用
    expect(fallback.paymentApp).toBeNull(); // フォールバックは null
    // base (finalAmount) / addOn (appBonusFinalAmount) / total は両分岐で一致 = 既存挙動不変。
    expect(fallback.finalAmount).toBeCloseTo(normal.finalAmount, 9);
    expect(fallback.appBonusFinalAmount).toBeCloseTo(normal.appBonusFinalAmount, 9);
    expect(fallback.totalFinalAmount).toBeCloseTo(normal.totalFinalAmount, 9);
    expect(fallback.appBonusBreakdown.map((b) => b.programId).sort()).toEqual(
      normal.appBonusBreakdown.map((b) => b.programId).sort(),
    );
    expect(fallback.totalFinalAmount).toBeCloseTo(2000, 6); // 両分岐とも実質 4%
  });
});
