...
ルール (card.prompt.md / point-partner.prompt.md と同一基準):
- 「ご利用期間: 2026年4月1日〜2026年6月30日」「YYYY年X月X日以降」等の **逐語的な日付** がある場合のみ抽出
- 開始日 + 終了日 両方明記 → `validFrom` + `validTo` 両方セット (= 期間限定キャンペーン)
- 開始日のみ明記、終了未告知 → `validFrom` のみ (= 長期公式プログラム、例: JAL特約店プログラム自体の開始日が明示されてる場合)
...
- evidenceQuote に「期間」「ご利用期間」「YYYY年」「まで」「以降」等の **日付根拠** が含まれない場合は validFrom/validTo を入れてはならない

`evidenceQuote` には**日付を含む箇所**を逐語引用すること。これがない場合は validFrom/validTo を省くこと。
