...
- **`scope`: 必須。キャンペーンは常に対象店舗を持つ (store membership 前提) ので必ず
  `"member-stores"` を指定する** (全店一律の施策はそもそもこの extractor の対象外で出力しない)。
...
- **`requiresEntry`**（任意 boolean、v3.5 で追加）: そのキャンペーンが**事前エントリー・
  参加登録・店ごとのポイントアップ登録などを必須とする**ことがページに**逐語で**書かれている
  場合のみ `true`。例:「要エントリー」「エントリーが必要」「事前エントリー」「◯◯登録が必要」。
  - **逐語根拠がある時だけ `true`**。書かれていなければフィールドごと省略する（`false` を
    default で埋めない）。UI が「⚠ 要エントリー」バッジ + `entryUrl` 起動に使うだけのメタで、
    rate 計算には影響しない（未エントリーでも計算値は据え置き）。
  - 「エントリーで N% (率不明)」のような**率が曖昧なキャンペーンはそもそも出力しない**（上の
    出力禁止パターン参照）。requiresEntry は「率・期間が確定した上で、加えてエントリーも要る」
    健全なキャンペーンに付ける補助フラグである。
...
{ "storeId": "newdays", "name": "NewDays", "category": "コンビニ",
  "evidenceQuote": "対象店舗 NewDays", "explicitness": 1.0, "ambiguity": 0.05 }
```
...
1. `programs[].rate` は必ず正の小数値。0／null／省略は不可。
   ...
   併せて `scope: "member-stores"` を必ず付ける (キャンペーンは常に対象店舗あり)。
   **`enabled` / `optIn` は出力しない** — これらはユーザー所有の preference キーで、
   seed / master には載せない (アプリ側が「使う」トグルで管理する)。
...
各項目に必須。`explicitness` は記述の直接度、`ambiguity` は曖昧さ。
confidence は `explicitness * (1 - ambiguity)` で機械算出され、キャンペーンの
自動マージ閾値は **≥ 0.90**。この閾値は「期間の逐語明記・rate≤30%・既存参照整合・
逐語引用」という全構造条件を満たした **後** の最終確認なので、構造条件を満たした
健全なキャンペーンは原則ここを通すべきものです。

**校正の指針（重要 — ここを誤ると健全なキャンペーンが全部レビュー送りになる）**:

- このプロンプトは「rate と期間の両方が `evidenceQuote` に逐語で出ているもの**だけ**を
  出力する」設計です。**その逐語根拠がページに直接書かれているなら `explicitness = 1.0`**
  と付けてください。逐語引用できているのに 0.9 を付けるのは過小評価です
  （`0.9 × (1-0.1) = 0.81 < 0.90` となり、正しいキャンペーンが自動反映から漏れます）。
- `ambiguity` は「複数解釈の余地」。対象店舗・還元率・期間が一意に定まるなら `0.0〜0.05`。
  ここを 0.1 にすると `1.0 × 0.9 = 0.90` ギリギリになるので、**本当に解釈が割れる時だけ**
  上げてください。
- **本当に曖昧なときだけ低く付ける**: 推測が混じる／対象店が不明瞭／率の根拠が間接的、
  といった場合は `explicitness` を下げる（→ 0.90 未満となり人手レビューに回る）のが正しい挙動。
- まとめると「逐語で裏取りできた事実を機械的に割り引かない。曖昧なものだけ正直に下げる」。
  過信は禁物ですが、**確実なキャンペーンを過小評価しない**ことが同じくらい重要です。
...
- v3.5 (2026-07-20): `programs[]` に任意 `requiresEntry` (boolean) を追加 (残改善 PR-4)。
  事前エントリー / 参加登録 / 店ごと登録が必須である旨がページに逐語で書かれている時のみ
  `true`。UI (CalcResultCard 展開ビュー) の「⚠ 要エントリー」バッジ + entryUrl 起動に使う
  表示メタで rate 計算には影響しない。逐語根拠が無ければ省略 (`false` を default 埋めしない)。
- v3.4 (2026-07-15): schema v7 トレイン最終化。`programs[]` に `scope` 出力を必須化
  (キャンペーンは常に `"member-stores"`)。`enabled` / `optIn` はユーザー所有 preference の
  ため出力しない旨を明記。
- v3.3 (2026-06-25): confidence 校正の自己矛盾を修正。旧版は模範例が
  `explicitness=0.9 / ambiguity=0.1` (=confidence 0.81) なのに「閾値 ≥0.9」と
  記載しており、模範例どおりに付けると健全なキャンペーンが必ず lowConfidence で
  レビュー送りになっていた (REVIEW_QUEUE の lowConfidence 過半数の主因)。逐語根拠が
  ある事実は `explicitness=1.0` とする校正指針を明文化し、模範例を 1.0/0.05 に更新。
  自動マージ閾値も 0.95→0.90 に緩和 (campaign の構造ゲートが既に強力なため。
  propose-helpers.ts `CAMPAIGN_AUTO_CONFIDENCE_THRESHOLD`)。
