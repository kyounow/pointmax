...
### 改善履歴
- v1.1 (2026-07-15): schema v7 トレイン最終化。`programs[]` に `scope: "member-stores"`
  出力を必須化 (たまるマーケットは常に対象ショップ membership ありのため)。`enabled` /
  `optIn` はユーザー所有 preference のため出力しない旨を明記。
...
**重要**:
- `scope: "member-stores"` を必須で付ける (対象ショップ membership を持つため)。
