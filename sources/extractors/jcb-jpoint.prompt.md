...
以下は既に `programs/memberships` に登録済 (V5 Phase 2 で手動投入)。
**重複追加しない**でください。ページ上で**これら以外**の提携店、または
**既存店の倍率が変動**している場合のみ抽出してください。

- **2倍 (prog-jcb-jpoint-2x)**: mercari / welcia / apollo-station / bic-camera / mos-burger
- **3倍 (prog-jcb-jpoint-3x)**: amazon / conv-7eleven
- **4倍 (prog-jcb-jpoint-4x)**: takashimaya (プレミアム条件付き、最大)
- **20倍 (prog-jcb-jpoint-20x)**: starbucks (モバイルオーダー / カードチャージ限定)

`programId` は上記 4 種を再利用。新しい倍率階層 (例: 5倍) があれば
`prog-jcb-jpoint-5x` 等の slug で新規 program を出力してください。
