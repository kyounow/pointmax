...
### 改善履歴
- v1.3 (2026-07-21): 四半期監査 2026-Q3 で J-POINT の付与が**加算方式**と確定
  (公式計算例 https://www.jcb.co.jp/ordercard/special/jpoint.html)。ショップ表記「N倍」は
  標準 0.5% 基準の合計倍率で、W (JCB CARD W) はカード特典 +1倍 が加算される。rate 算出を
  旧「W 1%×N / Gold 0.5%×N」の**乗算モデル**から「W (N+1)×0.5% / Gold N×0.5%」の**加算方式**に修正
  (スタバ 20倍店: 旧 W 20% → 正 10.5%)。
...
- **W 系列** (`cardIds=["jcb-w"]`、加算方式で実効 (N+1)×0.5%。id はショップ表記 N):
  - `prog-jcb-jpoint-2x` (rate 0.015 = 2倍店→計3倍): mercari / welcia / apollo-station / bic-camera / mos-burger / takashimaya
  - `prog-jcb-jpoint-3x` (rate 0.02 = 3倍店→計4倍): amazon / conv-7eleven
  - `prog-jcb-jpoint-20x` (rate 0.105 = 20倍店→計21倍): starbucks
  - (※ prog-jcb-jpoint-4x は廃止。高島屋は 2倍店として移管済)
- **Gold 系列** (`cardIds=["jcb-gold"]`、加算方式でも実効 N×0.5%):
...
ショップ表記「N倍」は標準 (0.5% = 200円1pt) 基準の**合計倍率**。付与は
「通常1倍 + カード特典倍率 + ショップ倍率」の**加算**で決まる (乗算ではない):
- W (JCB CARD W) はカード特典 +1倍 が加算される
  → `rate: (N + 1) × 0.005`、cardIds=["jcb-w"]、id=`prog-jcb-jpoint-{N}x` (id はショップ表記 N)
  例: 20倍店 → 計21倍 → rate 0.105 (10.5%)、2倍店 → 計3倍 → rate 0.015 (1.5%)
- Gold はカード特典倍率なし → `rate: N × 0.005`、cardIds=["jcb-gold"]、id=`prog-jcb-jpoint-gold-{N}x`
出典: https://www.jcb.co.jp/ordercard/special/jpoint.html (公式計算例。スタバ eGift 10,000円 =
通常50pt + Wボーナス50pt + パートナー950pt = 1,050pt 計21倍 10.5%)
...
倍率階層ごとに 1 件。実効 rate は加算方式で正規化 (W: `(N+1) × 0.005`、Gold: `N × 0.005`。N はショップ表記倍率):
...
```json
{
  ...
  "currencyId": "j-point",
  "bonusType": "primary",
  "description": "JCB J-POINT パートナー 3倍店 (W はカード特典+1倍で計4倍 = 実効 2.0%)",
