...
- 住宅ローン / カードローン契約
- 証券口座 (SBI証券 / 日興証券 等) の口座開設・取引・NISA保有資産・投資信託保有額
- 保険 / Vitality / 健康プログラム / Healthcare
- 旅行予約サービス経由の決済 (Vトリップ 等)
- その他「○○契約あり」「○○利用あり」「○○保有あり」等のステータス条件

**出力対象の判定基準**:

| 出力対象 ✅ | 出力対象外 ❌ |
|---|---|
| 特定店舗 × カード × paymentApp の還元アップ (例: セブン-イレブン タッチ 7%) | 給与振込口座指定 +3% |
| カード/Olive 保有のみで自動適用される常時優遇 | 円預金残高 250万以上 +0.5% |
| 「選べる特典」「カード Default 倍率 X 倍」等の単純 opt-in (注意: 選択前提なので conditions に明示) | 投資信託保有額 200万以上 +0.5% |
| 公式が「対象カード保有者全員に適用」と明記する施策 | Vitality 健康プログラム加入 +2% |

ライフスタイル系を発見した場合は `notes` フィールドに「ライフスタイル条件のため出力対象外: ○○ (給与振込/住宅ローン/保険/投資/etc.)」と記録し、programs[] / memberships[] には **追加しない**。

**例**: smbc-vpoint-up (https://www.smbc.co.jp/kojin/vpoint-up/) には 15 種類の還元アップ施策があるが、出力すべきは:
- ✅ 対象コンビニ・飲食店 タッチ決済 (店舗+paymentApp ベース)
- ✅ Olive 選べる特典 (シンプル opt-in、注意明記)
- ❌ Olive 給与振込 / 円預金残高 / 外貨預金 / 住宅ローン (ライフスタイル)
- ❌ SBI証券 / NISA保有資産 / 日興証券 (証券口座条件)
- ❌ Vitality / Healthcare (保険・健康)
- ❌ Vトリップ / カードローン (特定サービス経由のみ発動)
